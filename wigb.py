...
def wiggle(axobject,data, tt=None, xx=None, color='k', sf=0.15, verbose=False):
    ...
    for ntr in range(Ntr):
        ...
        axobject.fill_betweenx(tt_zi, offset, trace_zi + offset,
                         where=trace_zi >= 0,
                         facecolor=color)
        axobject.plot(trace_zi + offset, tt_zi, color)
    axobject.set_xlim(xx[0] - ts, xx[-1] + ts)
    axobject.set_ylim(tt[0], tt[-1])
    axobject.invert_yaxis()
